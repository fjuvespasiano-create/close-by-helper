// Scraper de telefones úteis e serviços públicos do site oficial de São José da Lapa.
...
export const scrapeSjlContacts = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (prioritized.length === 0) {
      return { contacts: [] as ScrapedSjlContact[], visited: [] as string[], errors: ["Nenhuma URL descoberta"] };
    }

    // 2) Batch scrape com extração JSON
    const errors: string[] = [];
    const contacts: ScrapedSjlContact[] = [];
