    ...
    new Promise<GPSDetectResult>((resolve) => {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          ...
          const map: Record<number, { reason: Reason; message: string }> = {
          ...
          const info = map[err.code] ?? { reason: "unavailable" as Reason, message: err.message || "Não foi possível obter sua localização." };
