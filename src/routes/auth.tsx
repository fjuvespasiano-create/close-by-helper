...
function AuthPage() {
  ...
  useEffect(() => {
    // Detect password-recovery deep-links: Supabase appends #type=recovery
    // to the callback URL. If present, we must NOT auto-redirect.
  ...
  useEffect(() => {
    const { data: sub } = supabase.auth.onAuthStateChange((event, session) => {
      // Never auto-redirect during password recovery flow.
      if (event === "PASSWORD_RECOVERY" || isRecoveringRef.current) {
        navigate({ to: "/reset-password", replace: true });
        return;
      }
...
function mapAuthError(message: string): string {
  ...
  if (m.includes("user already registered")) return "Este e-mail já tem cadastro. Faça login ou recupere a senha.";
  if (m.includes("rate limit")) return "Muitas tentativas. Aguarde alguns minutos e tente novamente.";
  if (m.includes("password should be")) return "Senha muito curta. Use pelo menos 6 caracteres.";
  return message;
}
