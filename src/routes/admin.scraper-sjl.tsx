...
import {
  scrapeSjlContacts,
  importScrapedSjlContacts,
  type ScrapedSjlContact,
} from "@/lib/scrape-sjl.functions";
import { categoryLabel } from "@/lib/publicServices";

export const Route = createFileRoute("/admin/scraper-sjl")({
  head: () => ({ meta: [{ title: "Scraper São José da Lapa — Admin" }, { name: "robots", content: "noindex" }] }),
  component: ScraperPage,
});

function ScraperPage() {
  const run = useServerFn(scrapeSjlContacts);
  const imp = useServerFn(importScrapedSjlContacts);
  const [keyword, setKeyword] = useState("telefone contato secretaria serviço");
  const [maxPages, setMaxPages] = useState(15);
  const [results, setResults] = useState<ScrapedSjlContact[]>([]);
  ...
  return (
    <div className="space-y-6">
      <header className="space-y-1">
        <h1 className="font-display text-2xl font-bold">Scraper — Telefones úteis de São José da Lapa</h1>
        <p className="text-sm text-muted-foreground">
          Extrai serviços e contatos públicos do site oficial{" "}
          <a href="https://www.saojosedalapa.mg.gov.br" target="_blank" rel="noreferrer" className="underline">
